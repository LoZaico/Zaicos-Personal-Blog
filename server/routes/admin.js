...
router.get('/admin', async (req, res) => {
    try {
      const locals = {
        title: "Admin",
        description: "Un Blog Simple creado con NodeJs, Express & MongoDb."
      }
  ...
  */
  ...
  router.get('/add-post', authMiddleware, async (req, res) => {
    ...
    try {
      const locals = {
        title: 'Add Post',
        description: 'Panel administrativo para crear publicaciones'
      }
...
router.post('/register', async (req, res) => {
  try {
    ...
    try {
      ...
      if(error.code === 11000) {
        res.status(409).json({ message: 'Usuario en Uso' });
      }
